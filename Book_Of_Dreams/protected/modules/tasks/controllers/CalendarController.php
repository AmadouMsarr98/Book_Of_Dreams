...
class CalendarController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        return $this->render('index', [
            ...
            'editUrl' => Url::to(['/tasks/task/edit'])
        ]);
    }
    ...
    public function actionSelect($start = null, $end = null)
    {
        ...
        if(!ContentContainerModuleManager::getDefaultState(Space::class, 'tasks')) {
            $calendarMemberSpaceQuery->leftJoin('contentcontainer_module',
                'contentcontainer_module.module_id = :tasks AND contentcontainer_module.contentcontainer_id = space.contentcontainer_id',
                [':tasks' => 'tasks']
        ...
        return $this->renderAjax('selectContainerModal', [
            ...
            'submitUrl' => Url::to(['/tasks/calendar/select-submit', 'start' => $start, 'end' => $end]),
        ]);
    }
    ...
    public function actionSelectSubmit($start = null, $end = null)
    {
        ...
        if ($container instanceof User && $container->is(Yii::$app->user->getIdentity())) {
            if (!$container->isModuleEnabled('tasks')) {
                return Yii::$app->runAction('/tasks/calendar/enable', ['start' => $start, 'end' => $end]);
        ...
        return Yii::$app->runAction('/tasks/task/edit', $params);
    }
    ...
    /*public function actionLoadAjax($start, $end)
    {
        ...
        if (!Yii::$app->user->isGuest) {
            ...
            $entries = CalendarService::getCalendarItems(new DateTime($start, CalendarUtils::getUserTimeZone()), new DateTime($end, CalendarUtils::getUserTimeZone()), $filters);
        } else {
            $entries = CalendarService::getCalendarItems(new DateTime($start, CalendarUtils::getUserTimeZone()), new DateTime($end, CalendarUtils::getUserTimeZone()));
        }
    ...
    public function actionEnable($start, $end)
    {
        ...
        $editUrl = $user->createUrl('/tasks/task/edit', ['start' => $start, 'end' => $end]);

        $cancelButton = ModalButton::cancel();
        $enableButton = ModalButton::primary(Yii::t('CalendarModule.base', 'Enable'))
            ->action('content.container.enableModule', $user->createUrl('/user/account/enable-module', ['moduleId' => 'tasks']));
