...
class ListController extends AbstractTaskController
{
    ...
    //liste tache par projet
    public function actionListeProject($id = NULL)
    {
        $id = $this->getprojectById($id);
        if(!$id){
            throw new HttpException(404);
        }
        else
        $tasks= Task::find()->where(['task_list_id'=>$id])->all(); 
        return $this->renderAjax('liste',
            ['tasks'=>$tasks]
        );
    }
