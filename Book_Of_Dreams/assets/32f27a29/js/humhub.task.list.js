            ...
            applyDropEvents: function(dropper) {
                ...
                dropper.addEventListener('drop', function(e) {
                   ...
                   // target.className = 'dropper'; // Application du style par défaut
                    ...
                    if (stateAfterDrop === 'col-sm-3 todo-kanban dropper') {
                        var status = 1;
                        changeStatus(status, idTask, 'kanban-panel-todo');

                    } else if(stateAfterDrop === 'col-sm-3 doing-kanban dropper') {
                       var status = 2;
                       changeStatus(status, idTask, 'kanban-panel-doing');

                    } else if(stateAfterDrop === 'col-sm-3 done-kanban dropper') {
                        var status = 5;
                        changeStatus(status, idTask, 'kanban-panel-done');
                    }

                    function changeStatus(status, idTask, statesDropper) {
                        var url = 'http://localhost:8888/humhub-1.3.15/dnd.php?idTask='+idTask+'&status='+status;                
                        xhr.open ('get', url, /*async*/ true);
                        xhr.send();
                        clonedElement.className = 'panel-body '+ statesDropper+' draggable';
                    }
